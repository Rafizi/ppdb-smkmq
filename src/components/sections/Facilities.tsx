  ...
  return (
    <section id="fasilitas" className="bg-background py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="reveal-up mt-8 flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card p-5">
          <span className="text-sm font-bold text-navy">Ekstrakurikuler:</span>
          {EXTRAS.map((extra, idx) => (
            <span
              key={extra}
              className="reveal-up rounded-full bg-primary/10 px-4 py-1.5 text-sm font-medium text-primary-dark"
              style={{ "--i": idx } as React.CSSProperties}
            >
              {extra}
            </span>
